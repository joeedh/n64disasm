...
export class lexer {
  ...
  clone() {
    let l = new lexer(this.tokdef);
    l.lexpos = this.lexpos;
    l.lexdata = this.lexdata;
    l.errfunc = this.errfunc;
    l.statestack = [];
    l.tokens = this.tokens;
    
    l.peeked_tokens = this.peeked_tokens.slice(0, this.peeked_tokens.length);
    
    for (let state of this.statestack) {
      l.statestack.push(state.slice(0, state.length));
    }
    
    l.statedata = this.statedata;
...
export class parser {
  ...
  clone(errfunc=this.errfunc) {
    let p = new parser(this.lexer.clone(), errfunc, this.start);
  ...
  expect(type, msg) {
    ...
    if (tok === undefined || tok.type != type) {
      if (!this.error(tok, "Expected " + msg)) {
        throw new PUTLParseError("Expected " + msg);
      }
    }
